...
from .model import import_models, make_model


__all__ = [
    'autocommit', 'execute', 'transaction',
    'Connection', 'DSN',
    'DictCursor', 'NamedTupleCursor', 'ReadCursor',
    'Error',
    'get_connection_pool',
    'import_models', 'make_model',
    'is_connection_error',
    'ping', 'table_exists',
    'ThreadConnectionPool',
]

DSN = collections.namedtuple('DSN', ('dbname', 'host', 'port', 'user',))
ERROR_DSN = DSN(*["error" for _ in DSN._fields])
...
def parse_dsn(s, _cache={}):
    """'dbname=foo host=bar user=zar' -> DSN(dbname='foo', host='bar', user='zar')
    """
    result = _cache.get(s)
    ...
    try:
        ...
        result = DSN(*[parsed.get(field) for field in DSN._fields])
    except Exception as e:
        result = ERROR_DSN
        log.exception("parse_dsn error dsn='{dsn}' error={e}", dsn=s, e=str(e))

    # Protect from eating all memory. Maybe weak dict could do better?
    if len(_cache) > 1000:
        _cache.clear()
...
class DictCursor(psycopg2.extras.DictCursor):
    ...
    def execute(self, statement, params=None, record_type=None):
        ...
        sql_id = id(sql)
        log.debug(u"Query [{time:.3f}] id={id} {sql}".format(
...
class NamedTupleCursor(psycopg2.extras.NamedTupleCursor):
    ...
    def execute(self, statement, params=None, record_type=None):
        ...
        sql = encoding.from_utf8(
            self.mogrify(statement, params)
            if not statement.lower().startswith("insert")
            else statement)
        sql_id = id(sql)
        log.debug(u"Query [{time:.3f}] id={id} {sql}".format(
...
class Connection(psycopg2.extensions.connection):
    ...
    def commit(self):
        ...
        log.debug(u"Commit [{time:.3f}]".format(time=total))

    def rollback(self):
        start = time.time()
        super(Connection, self).rollback()
        total = time.time() - start
        log.debug(u"Rollback [{time:.3f}]".format(time=total))
...
def get_connection_pool(group):
    config_dsn_key = 'DATABASE_{0}_DSN'.format(group).upper()
    dsn = flask.globals.current_app.config.get(config_dsn_key)
    assert dsn is not None, "Database is not configured. Expected key {0}".format(config_dsn_key)
...
@contextlib.contextmanager
def autocommit(group=GROUP_DEFAULT, connection_pool=None):
    """Context manager.
...
def execute(statement, params=None, group=GROUP_DEFAULT, connection_pool=None, repeat=True, record_type=None):
    """Shortcut for
...
@contextlib.contextmanager
def transaction(group=GROUP_DEFAULT, connection_pool=None):
    """Context manager.
...
def ping(quiet=False):
    try:
        ...
        if not quiet:
            log.critical('ping: {0}'.format(e))
        return False
    return True


def table_exists(name):
    sql = 'select 1 from {table} limit 1'.format(table=name)
    try:
        execute(sql)
