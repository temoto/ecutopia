def namedlist(typename, field_names, attrs=()):
    ...
    class ResultType(list):
        __slots__ = tuple(dict(attrs).keys())
        _fields = field_names
    ...
    ResultType.__init__ = eval(
        """lambda self, {fields}: [
            [setattr(self, name, default) for name, default in {attrs}.items()],
            self.__setitem__(slice(None, None, None), [{fields}]),
        ][-1]""".format(fields=fields_text, attrs=repr(attrs))
    )
